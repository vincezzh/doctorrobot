...
//            Doctor[] doctors = mapper.readValue(new File(Constant.JSON_OUTPUT), Doctor[].class);
            Doctor[] doctors = mapper.readValue(new File("/Users/vince/Downloads/doctors_bc.json"), Doctor[].class);
...
//                        recordCity(doctor.getLocation().getAddressSummary());
    ...
    private void recordDoctorJSon(Doctor doctor) throws Exception {
        ...
        log.info(doctorJson);
        FileUtil.appendContent(Constant.JSON_OUTPUT_FINAL_BC, doctorJson);
    }
    ...
    private void recordCity(String address) {
        if(address != null) {
            ...
            for (int i = 0; i < fields.length; i++) {
                if (i > 0 && "bc".equalsIgnoreCase(fields[i].trim())) {
